...
var zwave = new ZWave({
    ConsoleOutput: false,
    driverattempts: 100,        // try this many times before giving up
...
zwave.on('node ready', function (nodeid, nodeinfo) {
    ...
    console.log('---------------------------------------------------');    
    console.log(JSON.stringify(stateNodes));
    console.log('---------------------------------------------------');

    //console.log("nodeid = " + nodeid);
    if (nodeid == 336) {
        setInterval(function () {
            console.log("Press button (" + nodeid + ")");
            try {
                zwave.setValue(6, 37, 1, false);
                //zwave.pressButton({node_id: 6, class_id: 37, instance:1, index: 0}); 
            } catch (e) {
                console.log(e);
            }
        }, 3000);
